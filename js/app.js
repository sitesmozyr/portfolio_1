(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        const goUpIcon = document.getElementById("goUpIcon");
        document.addEventListener("scroll", (function(event) {
            if (window.scrollY > 400) goUpIcon.classList.add("_active"); else goUpIcon.classList.remove("_active");
        }));
        const asideLinks = document.querySelectorAll(".aside-bar__link");
        for (let index = 0; index < asideLinks.length; index++) {
            const asideLink = asideLinks[index];
            asideLink.addEventListener("click", (function(event) {
                for (let j = 0; j < asideLinks.length; j++) {
                    const asideLinkNotActive = asideLinks[j];
                    asideLinkNotActive.classList.remove("_active-link");
                }
                asideLink.classList.add("_active-link");
            }));
        }
        window["FLS"] = true;
        isWebp();
